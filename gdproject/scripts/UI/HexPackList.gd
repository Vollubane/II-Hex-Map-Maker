...
const RootTilesPackScene := preload("res://Scene/RootTilesPack.tscn")
...
func _ensure_packs_dir() -> void:
	...
	if da == null:
		push_error("HexPackList: cannot open user://")
		return
	if da.dir_exists("TilePack"):
		return
	var err := da.make_dir("TilePack")
	if err != OK:
		push_error("HexPackList: cannot create user://TilePack (%s)" % err)
...
func _list_pack_json_paths() -> Array[String]:
	...
	while entry != "":
		if not da.current_is_dir() and entry.ends_with(".json"):
			names.append(PACKS_DIR.path_join(entry))
		entry = da.get_next()
	da.list_dir_end()
	names.sort()
	return names
...
func _on_row_title_submitted(new_title: String, displayer: DataDisplayer) -> void:
	var old_path: String = displayer.get_meta("tile_pack_json_path", "")
	var stem := _sanitize_filename_stem(new_title)
	if stem.is_empty():
		displayer.set_title(_stem_from_path(old_path))
		return
	var new_path: String = PACKS_DIR.path_join(stem + ".json")
...
func _allocate_copy_stem(source_stem: String) -> String:
	for n in range(2, 100000):
		...
		var p := PACKS_DIR.path_join(candidate + ".json")
...
func _on_row_copy_pressed(displayer: DataDisplayer) -> void:
	var src: String = displayer.get_meta("tile_pack_json_path", "")
	var stem := _stem_from_path(src)
	var dest_stem := _allocate_copy_stem(stem)
	var dest := PACKS_DIR.path_join(dest_stem + ".json")
	if _copy_json_file(src, dest) != OK:
		push_error("HexPackList: copy failed")
		return
	_refresh_pack_list()


func _on_row_delete_pressed(displayer: DataDisplayer) -> void:
	var path: String = displayer.get_meta("tile_pack_json_path", "")
	if path.is_empty():
		return
...
func _on_row_play_pressed(displayer: DataDisplayer) -> void:
	...
	if path.is_empty():
		return
	RuntimeData.active_tile_pack_json_path = path
	var root := RootTilesPackScene.instantiate()
...
func _allocate_new_pack_stem() -> String:
	if not FileAccess.file_exists(PACKS_DIR.path_join("NewTilePack.json")):
		return "NewTilePack"
	for n in range(2, 100000):
		var s := "NewTilePack%d" % n
		if not FileAccess.file_exists(PACKS_DIR.path_join(s + ".json")):
			return s
	return "NewTilePack_%d" % Time.get_ticks_msec()


func _on_new_pack_pressed() -> void:
	var stem := _allocate_new_pack_stem()
	var path := PACKS_DIR.path_join(stem + ".json")
	var err := _write_json(path, _minimal_pack_dict())
	if err != OK:
		push_error("HexPackList: cannot create new tile pack json")
		return
	_refresh_pack_list()
